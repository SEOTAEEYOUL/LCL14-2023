...
def put_item(resource_id, check_dtm, check_result, check_result_detail):
    # DynamoDB 클라이언트 생성
...
def get_item(table_name="dynamodb_system_check_lcl14", resource_id='NETWORK-DX-01', check_dtm='2023/06/27 13:08:18'):
    # DynamoDB 클라이언트 생성
    dynamodb = boto3.client('dynamodb')
...
def get_items(table_name="dynamodb_system_check_lcl14"):
    # DynamoDB 클라이언트 생성
    dynamodb = boto3.client('dynamodb')
...
if __name__ == "__main__":
    ...
    resource_id = 'NETWORK-DX-01'
    check_dtm   = '2023/06/27 13:08:18'
    item = get_item(table_name, resource_id, check_dtm)
    pprint(item)