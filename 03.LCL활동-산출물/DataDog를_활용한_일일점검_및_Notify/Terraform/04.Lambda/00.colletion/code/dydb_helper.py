...
def put_item(table_name, check_dtm, resource_id, check_result, check_result_detail):
    # DynamoDB 클라이언트 생성
...
def get_item(table_name, check_dtm, resource_id):

    print(f"table_name[{table_name}], check_dtm[{check_dtm}], resource_id[{resource_id}]")
    # DynamoDB 클라이언트 생성
    dynamodb = boto3.client('dynamodb')
    ...
    if 'Item' in response:
        ...
        print("아이템 조회 결과:")
        print(f"check_dtm: {check_dtm}, resource_id: {resource_id}, check_result: {check_result}, check_result_detail: {check_result_detail}")
